...
import { setRoom as setGameRoom } from "../redux/ImprovedGameSlice"; // Also set room in game slice
  ...
  const handleCreateRoom = () => {
    if (Object.keys(selectedDeck).length !== 0) {
      if (socket.id) {
        ...
        dispatch(setRoom(roomNumber.toString())); // Old slice
        dispatch(setGameRoom(roomNumber.toString())); // NEW: Game slice
